...
type('dbref')
  ...
  .set( function (val, path) {
    ...
    if (this instanceof mongoose.Schema) {
      ...
      throw new Error("Context must be a Document or Schema instance");
    }
    var type = schema.paths[path]; // this can either be a document or schema
    ...
    if (val instanceof Document) {
      if (typeof val.id !== "undefined") {
        if (context === 'document') this._.dbrefs[path] = val;
        return new DBRef(val._schema._collection, val._.doc._id);
      } else {
    ...
    var promise = {
      do: function (fn) {
        if (this._.dbrefs[path]) {
          if (this._.dbrefs[path].id === this._.doc[path].oid.toHexString()) {
            fn(this._.dbrefs[path]);
          } else {
            this._schema.paths[path].subtype.findById(this._.doc[path].oid.toHexString(), function (err, record) {
              if (err) throw err;
              fn(record);
            });
          }
...
function DBReffedArray (docs, parent, path, hydrate) {
  ...
  this.parentReferredAs = parent._schema.paths[path].parentReferredAs;
}
// TODO Add in toJSON
