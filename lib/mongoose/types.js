...
type('dbref')
  ...
  /**
   * @param {Object|Document} val could be a hash or a Document or a DBRef instance
  ...
  .set( function (val, path) {
    ...
    if (val.namespace && val.oid) { // replaces instanceof DBRef, which doesn't seem to work
      this._.doc[path] = val;
      return val;
    } else {
    ...
    var promise = {
      do: function (fn) {
        if (self._.dbrefs[path]) {
          ...
          self._schema.paths[path].subtype.findById(self._.doc[path].oid.toHexString(), function (err, record) {
            if (err) throw err;
            self.set(record);
            fn(err, record);
          });
...
PromiseArray.prototype = {
  ...
  toJSON: function (fn) {
    var newPromiseArray = new PromiseArray();
    newPromiseArray.callback(newPromiseArray._toJSON, arguments);
    this.callback(newPromiseArray.succeed.bind(newPromiseArray));
    return newPromiseArray;
  },
  _toJSON: function (fn) {
...
function DBRefArray (docs, parent, path, hydrate) {
  this.parent = parent;
  this.path = path;
  var memberType = this.memberType = parent._schema.paths[path].subtype, // The dbref array's members' type
      arr = this.arr = [];
  var i, l;
  if (docs) {
    if (hydrate) {
...
DBRefArray.prototype = {
  ...
  _fetch: function () {
    ...
    var toFetch = this._whatToFetch()
      , idsToFetch = toFetch.map( function (el) {
          var id = el[1];
      ...
      , self = this;
    // TODO Does $in query return the objects in the same order as the idsToFetch ordering?
    memberType.find({_id: { $in: idsToFetch } }).all( function (err, docs) {
      var cb;
      self.state = 'fetched';
      self.arr = docs;
      self.err = err;
      ...
      while (cb = self.callbacks.shift()) {
        cb[0].apply(self, cb[1]); // Pass in the array if we didn't specify args
      }
    });
  },
...
// TODO splice, _splice
['at', 'all', 'forEach', 'slice', 'splice', 'filter', 'map', 'toJSON'].forEach( function (name) {
  DBRefArray.prototype[name] = PromiseArray.prototype[name];
  DBRefArray.prototype['_' + name] = PromiseArray.prototype['_' + name];
});
...
type('dbrefArray')
  ...
  .set( function (val, path) {
    ...
    for (var i = 0, len = val.length; i < len; i++) {
      ...
      if (! (member instanceof Document) ) throw new Error("Argument Error");
      if (typeof member.id === "undefined") throw new Error("The member type for a DBRef collection must have an oid");
      val[i] = member;
    }
...
function DBReffedArray (docs, parent, path, hydrate) {
  ...
  var type = parent._schema.paths[path]
    , opts = type.options;
  if (docs && docs.length) {
    this.state = 'set';
  } else if (docs === null) {
  ...
  this.parentReferredAs = opts.as;
  var sort = opts.sort;
  if (sort) {
    if (!Array.isArray(sort)) {
      // Convert {a: 1, b: -1} to [['a', 1], ['b', -1]]
      this.sort = Object.keys(sort).map( function (key) {
        return [key, sort[key]];
      });
    } else {
      this.sort = opts.sort;
...
DBReffedArray.prototype.create = function (attrs, fn) {
  attrs[this.parentReferredAs] = this.parent;
  var self = this;
  this.memberType.create(attrs, function (err, member) {
    if (!err) self.arr.push(member);
    if (fn) fn(err, member);
    else if (err) throw err;
  });
  return this;
};
DBReffedArray.prototype.override = function (newVal) {
  var self = this;
  this._remove = this.arr;
  this.arr.forEach( function (el, i) {
    self.set(i, el);
  });
};
DBReffedArray.prototype.destroyOldReferences = function (callback) {
  var count = 0
    , self = this;
  if (this._remove) this._remove.forEach( function (member) {
    var fieldsObj = {}
      , unsetObj = {};
    fieldsObj[self.parentReferredAs] = 1;
    unsetObj[self.parentReferredAs] = 1;
    member._collection.findAndModify({'_id': member._id}, [], {'$unset': unsetObj}, {'new': true, fields: fieldsObj}, function (err, doc) {
      if (doc) delete member._.doc[self.parentReferredAs];
      if (++count === self._remove.length) {
        delete self._remove;
...
//  conditions[this.parentReferredAs] = new DBRef(this.parent._schema._collection, this.parent._.doc._id);
  ...
  query.all( function (err, members) {
    if (err) throw err;
    var arr = self.arr;
    // Append any local items that weren't returned in the query to the end of the results
    // These local items would be new, unsaved items
    for (var i = 0, l = arr.length; i < l; i++) {
      var member = arr[i];
      var alreadyExists = members.some( function (el) {
        return el.id === arr[i].id;
      });
      if (!alreadyExists) {
        members.push(arr[i]);
      }
...
type('dbreffedArray')
  // TODO Take care of post-saves where we should remove our reference to "this"
  // TODO ... in the document we "removed" from our dbreffedArray
  .setup( function (key, path) {
    // Save the documents that refer to the parent document
    // after the parent document is saved
    this.post('save', function (next, done) { /** this is a Document instance **/
      var dbreffedPaths = Object.keys(this._.dbreffedArrays)
        , i = dbreffedPaths.length, dbreffedArr, toSave, count = 0;
      while (i--) {
        dbreffedArr = this._.dbreffedArrays[dbreffedPaths[i]];
        toSave = dbreffedArr.arr.filter( function (member) {
          return member.isNew;
        });
        toSave.forEach( function (member, j) {
          member.save( function (err, created) {
            if (err) next(err);
            else if (++count === toSave.length) next();
          });
        });

        dbreffedArr.destroyOldReferences();
      }
    });
  })
  // Same as dbrefArray's set
  // TODO Eliminate duplication?
  .set( function (val, path) { /** @scope is Document instance **/
    if (!(val instanceof Array)) throw new Error("You must pass in an array.");
    var type = this._schema.paths[path],
        subtype = type.subtype,
        member;
