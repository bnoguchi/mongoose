...
Document.prototype._getDirty = function(){
  var obj = {}
    , _dirty = Object.keys(this._.dirty);
    
  for(var i=0,l=_dirty.length; i<l; i++){
    obj[_dirty[i]] = this._get(_dirty[i]);
  }
  return obj;
};
...
Document.prototype._run = function(name, fn, args){
  ...
  if(name == 'save'){
    this._validate(override, next, args);
  } else { 
    next();
  }
};
...
var EmbeddedArray = function(arr, path, subtype, scope, hydrate){
  ...
  //      validators: schema.validators // The validators for the TypeSchema
    ...
    if(hydrate){
      if(this._.doc) for(var i=0,l=arr.length; i<l; i++) this.arr[i] = new this._.doc(arr[i]);
      else for(var i=0,l=arr.length; i<l; i++) this.arr[i] = arr[i];
    } else {
 ...
 //   this._.scope._.doc[this._.path] = this._.scope._.doc[this._.path] || this;
  ...
  set: function(idx, val){
    if(this._.doc){ // If we have a subtype (Schema)
      ...
      this.arr[idx] = this._.docs[idx]._.doc;
    } else {
      this._set(idx, val);
    }
